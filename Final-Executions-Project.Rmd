...
In our random forest model, we attempt to predict between lethal injection, hanging, firing squad, gas chamber, and electrocution.
...
>>>>>>> 37c6e98ce8c5084f0210d50052be7964d5afdb42
# fit models
...
Our outcome variable is the method of execution, since it is a categorical variable we will use a classification tree to answer our research question. To limit bias we will specifically use a random forest and the bootstrap method, to improve the statistical learning method. 
...
Overall, the out of bag error shows that the model that uses 12 variables has the greatest accuracy at 97%. Year is the most important variable for classification, which makes sense because different methods of execution were used at different times. State is the second most important variable, as method of execution varies depending on the state the execution takes place in. Federal and Crime type are the least important, which also makes sense because there are only two federal executions in the dataset and all crimes are murder. 
