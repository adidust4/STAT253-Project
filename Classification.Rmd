...
set.seed(123) 
data_fit_mtry74 <- fit(data_wf_mtry74, data = executions_clean)

set.seed(123)
data_fit_mtry147 <- fit(data_wf_mtry147, data = executions_clean)
```
...
rf_OOB_output(data_fit_mtry2,'mtry2', executions_clean %>% pull(class))
```

```{r}
ata_rf_OOB_output <- bind_rows(
    rf_OOB_output(data_fit_mtry2,'mtry2', executions_clean %>% pull(class)),
    rf_OOB_output(data_fit_mtry12,'mtry12', executions_clean %>% pull(class)),
    rf_OOB_output(data_fit_mtry74,'mtry74', executions_clean %>% pull(class)),
    rf_OOB_output(data_fit_mtry147,'mtry147', executions_clean %>% pull(class))
)
...
>>>>>>> parent of 2ecfe66 (Merge branch 'main' of https://github.com/adidust4/STAT253-Project into main)
```
...
data_wf_mtry21 <- workflow() %>% add_model(rf_spec %>% set_args(mtry = 21)) %>% add_recipe(data_rec)
```
...
set.seed(123) 
data_fit_mtry21 <- fit(data_wf_mtry21, data=executions_clean)
```
...
data_rf_OOB_output <- bind_rows(
    ...
    rf_OOB_output(data_fit_mtry21,21, executions_clean %>% pull(Method))
)
...
Overall, the out of bag error shows that the model that uses 12 variables has the greatest accuracy at 98.4%. Year is the most important variable for classification, which makes sense because it dictates customs and rules, and Crime and Federal are the least important which is unexpected. 
