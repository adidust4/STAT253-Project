...
executions_clean$Year <-
  as.numeric(as.character(executions_clean$Year))
...
execute_rec <- recipe(Victim_Count~., data = executions_clean) %>%
  step_filter(Victim_Count < 50) %>% # this doesn't seem to be working... not sure why
  step_cut(Month, breaks=c(1, 3, 8, 12)) %>%
  step_cut(Year, breaks=c(1970, 1980, 1990, 2000, 2010, 2020)) %>%
...
mod_output %>%
  ggplot(aes(x = .pred, y = resid)) +
    geom_point() + 
    geom_smooth() +
    ylim(0,10) + #if we can get filter to work in the recipe we can delete this
    theme_classic()
